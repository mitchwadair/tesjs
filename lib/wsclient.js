...
class WebSocketClient {
    ...
    _resetKeepaliveTimeout(id) {
        const connection = this.connections[id];
        clearTimeout(connection.keepaliveTimeout);
        connection.keepaliveTimeout = setTimeout(() => {
            EventManager.fire("connection_lost", connection.subscriptions);
        }, connection.keepaliveTimeoutLength);
    }
            ...
            switch (message_type) {
                case "session_welcome":
                    ...
                    this.connections[id] = {
                        client: ws,
                        subscriptions: {},
                        keepaliveTimeoutLength: keepalive_timeout_seconds * 1000,
                        keepaliveTimeout: setTimeout(() => {}),
                    };
                    this._resetKeepaliveTimeout(id);
                    resolve(id);
                    break;
                default:
