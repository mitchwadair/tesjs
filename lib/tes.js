...
class TES {
    constructor(config) {
        ...
        const handlerConfig = {
            ignoreDuplicateMessages: config.options.ignoreDuplicateMessages === false ? false : true,
            ignoreOldMessages: config.options.ignoreOldMessages === false ? false : true,
        }
        ...
        if (config.webhook) {
            ...
            if (!config.webhook.baseURL)
                throw new Error("TES webhook config must contain 'baseURL'");
            if (!config.webhook.secret)
                throw new Error("TES webhook config must contain 'secret'");
