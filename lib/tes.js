...
/**
 *  If you already have an authentication solution for your app elsewhere use this to avoid token conflicts
 *
 * @callback onAuthenticationFailure
 * @returns {Promise<string>} Promise that resolves a new access token
 * @example
 * ```js
...
class TES {
     ...
     *
     * @typicalname TES
     * @param {Object} config The TES configuration
     * @param {Object} [config.options] Basic configuration options
     * @param {boolean} [config.options.debug=false] Set to true for in-depth logging
     * @param {boolean} [config.options.logging=true] Set to false for no logging
     * @param {Object} config.identity Identity information
     * @param {string} config.identity.id Your client ID
     * @param {string} [config.identity.secret] Your client secret, required for webhook transport or
     *     when not using `onAuthenticationFailure` in server-side `websocket` applications
     * @param {onAuthenticationFailure} [config.identity.onAuthenticationFailure] If you already have an
     *     authentication solution for your app elsewhere use this to avoid token conflicts
     * @param {string} [config.identity.accessToken] If you already have an access token, put it here. Must
     ...
     * @param {string} [config.identity.refreshToken] The refresh token to use if using `websocket` transport
     *     server-side. Required when not using `onAuthenticationFailure` in server-side `websocket` applications
     * @param {Object} config.listener Your notification listener details
     * @param {"webhook"|"websocket"} config.listener.type The type of transport to use
     * @param {string} [config.listener.baseURL] Required for `webhook` transport. The base URL where your app is
     *     hosted. See [Twitch doc](https://dev.twitch.tv/docs/eventsub) for details on local development
     * @param {string} [config.listener.secret] Required for `webhook` transport. The secret to use for your `webhook`
     *     subscriptions. Should be different from your client secret
     * @param {Express} [config.listener.server] The Express app object. Use if integrating with an existing Express app
     * @param {boolean} [config.listener.ignoreDuplicateMessages=true] Ignore event messages with IDs that have already
     *     been seen. Only used in `webhook` transport
     * @param {boolean} [config.listener.ignoreOldMessages=true] Ignore event messages with timestamps older than ten
     ...
     * @param {string} version (optional) the version of the event subscription
     */
