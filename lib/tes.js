...
class TES {
    constructor(config) {
        ...
        if (config.webhook) {
            ...
            if (!config.identity.secret) throw new Error("TES identity config must contain 'secret' for webhooks");
            ...
            this.whSecret = config.webhook.secret;
            ...
            if (config.identity.secret && !config.identity.auth)
                throw new Error("TES identity config must contain 'auth'");
    ...
    _refreshAccessToken() {
        return new Promise((resolve, reject) => {
            ...
            if (this.authToken) {
                this._validateAccessToken(this.authToken)
                    .then(() => {
