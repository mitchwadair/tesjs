    ...
    subscribe(type, condition) {
        ...
        return new Promise((resolve, reject) => {
            this._refreshAppAccessToken().then(() => {
                ...
                request("POST", SUBS_API_URL, headers, body)
                    .then((data) => {
            ...
            this._refreshAppAccessToken()
                .then(() => {
                    ...
                    if (arguments.length === 1) {
                        ...
                        request("DELETE", `${SUBS_API_URL}?id=${id}`, headers).then(resolve).catch(reject);
                    } else if (arguments.length === 2) {
                        const type = args[0];
                        ...
                        this.getSubscription(type, condition).then((res) => {
                            if (res) {
                                request("DELETE", `${SUBS_API_URL}?id=${res.id}`, headers).then(resolve).catch(reject);
                            } else {
