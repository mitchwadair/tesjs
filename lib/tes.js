...
class TES {
    ...
    async subscribe(type, condition) {
        ...
        await this._refreshAppAccessToken();
        const headers = {
        ...
        const res = await fetch(SUBS_API_URL, { method: "POST", body: JSON.stringify(body), headers });
    ...
    async unsubscribe(...args) {
        ...
        const unsub = async (id) => {
            const res = await fetch(`${SUBS_API_URL}?id=${id}`, { method: "DELETE", headers });
    ...
    async _refreshAppAccessToken() {
        const getNewToken = async () => {
            logger.debug("Getting new app access token");
            const res = await fetch(
                `${AUTH_API_URL}/token?client_id=${this.clientID}&client_secret=${this.clientSecret}&grant_type=client_credentials`,
                { method: "POST" }
            );
            const data = await res.json();
        ...
        if (this._authToken) {
            try {
                await this._validateAppAccessToken(this._authToken);
