...
const whserver = require("./deliveries/wehbooks/whserver");
...
class TES {
    constructor(config) {
        ...
        // ensure we have an identity (the client id)
        if (!config.identity) throw new Error("TES config must contain 'identity'");
        this.clientID = config.identity;
    ...
    subscribe(type, condition) {
        ...
        return new Promise((resolve, reject) => {
            this._refreshAppAccessToken().then(() => {
                ...
                request("POST", "https://api.twitch.tv/helix/eventsub/subscriptions", headers, body)
                    .then((data) => {
            ...
            this._refreshAppAccessToken()
                .then(() => {
                    ...
                    if (arguments.length === 1) {
                        ...
                        request("DELETE", `https://api.twitch.tv/helix/eventsub/subscriptions?id=${id}`, headers)
                            .then(() => {
                                resolve();
                            })
                            .catch((err) => {
                        ...
                        this.getSubscription(type, condition).then((res) => {
                            if (res) {
                                request(
                                    "DELETE",
                                    `https://api.twitch.tv/helix/eventsub/subscriptions?id=${res.id}`,
                                    headers
                                )
                                    .then(() => {
                                        resolve();
    ...
    _validateAppAccessToken() {
        return new Promise((resolve, reject) => {
            ...
            request("POST", "https://id.twitch.tv/oauth2/validate", headers)
                .then((data) => {
